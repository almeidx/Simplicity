...
class ImageArgument extends Argument {
  ...
  static async parse(arg, context) {
        ...
        try {
          ...
          if (user) {
            try {
              ...
              const buffer = await imageRequest(user.displayAvatarURL({ dynamic: true }), client);
              return buffer;
            } catch (e) {
              client.logError(e);
              throw new CommandError(t('errors:imageParsingError'));
    ...
    if (this.authorAvatar) {
      try {
        parseState.argIndex--;
        if (this.url) return author.displayAvatarURL({ dynamic: true });
        const buffer = await imageRequest(author.displayAvatarURL({ dynamic: true }), client);
        return buffer;
      } catch (e) {
        client.logError(e);
        throw new CommandError(t('errors:imageParsingError'));
