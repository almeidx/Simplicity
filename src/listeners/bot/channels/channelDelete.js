...
class ChannelDelete extends SimplicityListener {
  constructor (client) {
    super(client)
  }
  ...
  async on (_, channel) {
    ...
    const embed = new SimplicityEmbed(this.getFixedT(guild.id))
      ...
      .setDescription('loggers:channelDeleted', { type: channel.type, name: channel.name })
    ...
    if (guild.me.permissions.has('VIEW_AUDIT_LOG')) {
      ...
      if (entry && (entry.target && entry.target.id === channel.id) && entry.createdTimestamp > Date.now() - 5000) {
        ...
        if (executor)
          embed.setDescription('loggers:channelDeletedExecutor', { type: channel.type, name: channel.name, executor })
      }
    }
