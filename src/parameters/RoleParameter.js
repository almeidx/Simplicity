...
class RoleParameter extends Parameter {
  ...
  static verifyExceptions (role, exceptions = {}, { guild }) {
    ...
    if (!exceptions.canBeBotRole && !role.editable && higherRole)
      throw new CommandError(exceptions.errors.canBeBotRole, { onUsage: true })
    if (!exceptions.canBeHigher && higherRole)
      throw new CommandError(exceptions.errors.canBeHigher, { onUsage: true })
    if (!exceptions.canBeLower && lowerRole)
      throw new CommandError(exceptions.errors.canBeLower, { onUsage: true })
