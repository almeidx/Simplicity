...
class UserInfo extends Command {
  ...
  async run ({ author, client, channel, emoji, guild, query, send, t }) {
    ...
    if (memberPermissions) {
      resultAdministrator = memberPermissions.includes(ADMINISTRATOR_PERMISSION) && t('permissions:' + ADMINISTRATOR_PERMISSION)
      resultAllPermissions = memberPermissions.sort((a, b) => PERMISSIONS.indexOf(a) - PERMISSIONS.indexOf(b))
      resultPermissions = resultAdministrator || (resultAllPermissions && resultAllPermissions.map(p => t('permissions:' + p)).join(', '))
    }
    ...
    if (checkPermissions('ADD_REACTIONS') && checkPermissions('READ_MESSAGE_HISTORY')) {
      const spotifyRestriction = activity && activity.party && activity.party.id && activity.party.id.includes('spotify:')
      let spotify, role
      ...
      if (spotify || role) {
        ...
        collector.on('collect', async ({ emoji, users, message }) => {
          ...
          if (checkPermissions('MANAGE_MESSAGES'))
            await users.remove(user)
          if (spotify && spotify.emoji === name && !checkEmbed(spotify.embed))
            await message.edit(spotify.embed)
          if (name === userinfoEmoji && !checkEmbed(embed))
            await message.edit(embed)
          if (role && role.emoji === name && !checkEmbed(role.embed))
            await message.edit(role.embed)
        })

        collector.on('end', async () => {
          if (message && checkPermissions('MANAGE_MESSAGES'))
