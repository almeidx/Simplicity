...
class CreateEmoji extends Command {
  ...
  run ({ guild, author, send, t, args }) {
    ...
    const url = args.join('')
    if (!name.length >= 32) {
      embed.setDescription(t('commands:createemoji.nameTooBig'))
      return send(embed, { error: true })
    } else if (!name.length >= 2) {
      embed.setDescription(t('commands:createemoji.nameTooShort'))
      return send(embed, { error: true })
    } else if (!name.match('(^[a-zA-Z0-9_]*$)')) {
      embed.setDescription(t('commands:createemoji.invalidName'))
      return send(embed, { error: true })
    } else if (url.startsWith('http://' || 'https://') &&
              url.includes('.png' || '.gif' || '.webp' || '.jpg' || '.jpeg')) {
      embed.setDescription(t('commands:createemoji.invalidURL'))
      return send(embed, { error: true })
    } else {
      guild.emojis.create(url, name)
        ...
        .catch(() => {
          embed.setDescription(t('commands:createemoji.error'))
          return send(embed, { error: true })
        })
    }
  }
