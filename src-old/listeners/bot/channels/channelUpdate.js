...
const { SimplicityEmbed, SimplicityListener, Utils: { cleanString, getServerIconURL } } = require('../../../index');
...
class ChannelUpdate extends SimplicityListener {
  ...
  async on(_, oldChannel, newChannel) {
    ...
    const embed = new SimplicityEmbed(this.getFixedT(guild.id))
      .setTimestamp()
      .setAuthor(guild.name, getServerIconURL(guild))
    ...
    if (guild.me.permissions.has('VIEW_AUDIT_LOG')) {
      ...
      if (entry && (entry.target && entry.target.id === newChannel.id) &&
        entry.createdTimestamp > Date.now() - 5000) executor = entry.executor;
    }

    // NAME CHANGES
    if (oldChannel.name !== newChannel.name) {
      if (executor) embed.setDescription('loggers:channelNameChangedExecutor', {
        type: newChannel.type,
        oldName: oldChannel.name,
        newName: newChannel.name,
        executor,
      });
      else embed.setDescription('loggers:channelNameChanged', {
        type: newChannel.type,
        oldName: oldChannel.name,
        newName: newChannel.name,
      });
